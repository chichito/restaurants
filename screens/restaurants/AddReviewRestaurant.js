...
import { addDocumentWithoutId, getCurrecntUser, getDocumentById, updateDocument } from '../../utils/actions'
...
export default function AddReviewRestaurant({ navigation, route }) {
    ...
    const addReview = async() => {
        ...
        if(!responseUpdateRestaurant.statusResponse){
            setLoading(false)
            toastRef.current.show("Error al enviar la actualizacion del restaurant, por favor intenta mas tarde.",3000)
            return
        }
        navigation.goBack()
    ...
    const validateForm = () => {
        ...
        if(!rating){
            toastRef.current.show("Debes darle una puntuaacion al restaurant.",3000)
            isValid = false
        }
        if(isEmpty(title)){
        ...
        if(isEmpty(review)){
            setErrorReview("Debes ingresar un titulo a tu comentario.")
            isValid = false
        }
        return isValid
    }
